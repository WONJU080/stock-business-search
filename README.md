**自娱自乐vibe coding第三弹。较第二弹的那个，大大加强了实用性。将关键词检索改善。将数据库改善。后续，为了发挥与ai聊天不同的高效率特点，需要改善ai分析财报的逻辑。以及制作可以脱离出程序之外并行分析不同财报的数据获取工具。否则以目前这个速度得猴年马月才能实现预想中“快人一步找到目标股票”的效果。那还不如看券商的推送。
